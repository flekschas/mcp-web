<script lang="ts">
  import { state, resetGame } from '../mcp.js';

  $: capturedRed = state.gameState.capturedPieces.red;
  $: capturedBlack = state.gameState.capturedPieces.black;
...
<div class="bg-gray-800 text-white p-4 rounded-lg space-y-4">
    ...
    {#if gameStatus === 'playing'}
      <p class="text-lg">
        ...
        {:else if currentTurn === 'red'}
          <span class="text-red-400">Your turn (Red)</span>
        {:else}
          <span class="text-blue-400">AI turn (Black)</span>
        {/if}
      </p>
    {:else if gameStatus === 'red_wins'}
  ...
  <div class="grid grid-cols-2 gap-4 text-sm">
    ...
    <div class="text-center">
      ...
      <div class="text-white font-semibold">
        ...
        <span class="text-blue-400">{capturedRed}</span>
      </div>
    </div>
  </div>
  ...
  <div class="text-xs text-gray-400 space-y-1">
    ...
    <div>• You play as red (bottom), AI plays as black (top)</div>
