...
export function createInitialBoard(): PieceType[][] {
  ...
  // Place red pieces (human) on bottom rows (5, 6, 7)
...
export function isRed(piece: PieceType): boolean {
  return piece === RED_PIECE || piece === RED_QUEEN;
}
...
export function isQueen(piece: PieceType): boolean {
  return piece === RED_QUEEN || piece === BLACK_QUEEN;
}
...
export function canPieceMoveTo(
  ...
  if (Math.abs(rowDiff) === 1 && Math.abs(colDiff) === 1) {
    ...
    if (!isPieceQueen) {
      if (player === 'red' && rowDiff > 0) return false; // Red moves up (decreasing row)
  ...
  if (Math.abs(rowDiff) === 2 && Math.abs(colDiff) === 2) {
    ...
    if (!isPieceQueen) {
      if (player === 'red' && rowDiff > 0) return false;
...
export function makeMove(state: GameState, move: Move): GameState {
  ...
  if (piece === RED_PIECE && to.row === 0) finalPiece = RED_QUEEN;
  ...
  if (isCaptureMove(state.board, move)) {
    ...
    for (let i = 1; i < distance; i++) {
      ...
      if (capturedPiece !== EMPTY) {
        ...
        if (isRed(capturedPiece)) {
          newCapturedPieces.red++;
...
export function checkGameStatus(
  ...
  if (legalMoves.length === 0) {
    ...
    return state.currentTurn === 'red' ? 'black_wins' : 'red_wins';
  }
