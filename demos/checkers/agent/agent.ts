...
// Initialize MCP client with AUTH_TOKEN from environment
...
function initializeModel() {
  ...
  switch (provider) {
    ...
    case 'cerebras':
      return createCerebras()(modelName);
  }
}
...
async function generateStructuredAnswer<T>({
  ...
  const contextGathering = await generateText({
    ...
    Your output will be provided to another agent that will formulate the final answer.
    `),
    prompt: query.prompt,
    tools: aiSdkTools,
