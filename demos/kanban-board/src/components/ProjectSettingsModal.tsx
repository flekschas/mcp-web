...
const ProjectSettingsModal = ({
  ...
  const isAdmin = currentUser.role === 'admin' || currentUser.role === 'product-manager';
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-lg shadow-xl w-full max-w-lg mx-4">
        ...
        <div className="px-6 py-4">
          {!canEdit && (
            <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
              <p className="text-sm text-amber-800 dark:text-amber-200">
                <strong>View Only:</strong> You don't have permission to edit this project. Only the project owner or admins can make changes.
              </p>
            </div>
          )}
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="project-name" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">
                Project Name
              </label>
              <input
                id="project-name"
            ...
            <div>
              <label htmlFor="project-description" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">
                Description
              </label>
              <textarea
            ...
            <div>
              <label htmlFor="project-deadline" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">
                Deadline (Optional)
              </label>
              <input
                id="project-deadline"
            ...
            <div>
              <label htmlFor="project-status" className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">
                Status
              </label>
              <select
                id="project-status"
          ...
          <div className="mt-6 text-xs text-zinc-500 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-800 p-3 rounded-lg">
            <p className="font-medium mb-1">Project Owner: {project.teamMembers.find(m => m.id === project.ownerId)?.name || 'Unknown'}</p>
