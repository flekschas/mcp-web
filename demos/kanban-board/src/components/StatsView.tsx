...
const StatsView = ({
  ...
  const assigneeWorkload = useMemo(() => {
    return teamMembers.map((member) => ({
      ...member,
      taskCount: filteredTasks.filter(task => task.assigneeId === member.id).length,
      completedCount: filteredTasks.filter(task => task.assigneeId === member.id && task.status === 'done').length,
    }))
  ...
  const workloadData = useMemo(() => {
    return assigneeWorkload.map((member) => ({
      ...
      completionRate: member.taskCount > 0 ? Math.round((member.completedCount / member.taskCount) * 100) : 0
    }))
  }, [assigneeWorkload]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 p-6">
        ...
        {/* Detailed Team List */}
        <h4 className="text-md font-medium text-zinc-800 dark:text-zinc-200 mb-3">Detailed Breakdown</h4>
        <div className="space-y-4">
            ...
            return (
              <div key={member.id} className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-700 rounded-lg">
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm text-zinc-500 dark:text-zinc-400">
                    {completionRate}% completed ({member.completedCount}/{member.taskCount})
                  </div>
                </div>
              </div>
            );
          ...
          {filteredTasks.some(task => !task.assigneeId) && (
            <div className="flex items-center justify-between p-4 bg-zinc-50 dark:bg-zinc-700 rounded-lg">
              ...
              <div className="text-right">
                <div className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
                  {filteredTasks.filter(task => !task.assigneeId).length} tasks
                </div>
                <div className="text-sm text-zinc-500 dark:text-zinc-400">
                  Need assignment
                </div>
