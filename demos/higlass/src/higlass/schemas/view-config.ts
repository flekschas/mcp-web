 ...
 */
export const viewportProjectionTypeSchema = z
  .enum([
  ...
  .describe(
    'Viewport projection track types for displaying projections of the main view',
  );

/**
 * Track type enums for standard HiGlass tracks
 */
export const standardTrackTypeSchema = z
  .enum([
    'multivec',
    '1d-heatmap',
    'line',
 ...
 */
export const trackTypeSchema = z
  .union([viewportProjectionTypeSchema, standardTrackTypeSchema])
  .describe(
    'Specifies the track type (e.g., "horizontal-heatmap", "horizontal-bar", "2d-heatmap").',
  );
...
export const viewLayoutSchema = z
  ...
  .describe(
    'Defines the spatial arrangement and dimensions of views in the visualization.',
  );

/**
 * Data object schema for tracks
 */
export const trackDataSchema = z
  .object({
    type: z.string().optional().describe('Data type identifier'),
    url: z.string().optional().describe('URL endpoint for track data'),
    server: z
      .string()
      .optional()
      .describe('Server URL providing the track data.'),
    filetype: z.string().optional().describe('File format type for the data'),
    children: z
      .array(z.any())
      .optional()
  ...
  .optional()
  .describe(
    'Configuration object containing track-specific display and behavior options.',
  );
...
export const baseTrackSchema = z.object({
  ...
  uid: z
    ...
    .optional()
    .describe(
      'Unique identifier for the view or track. Used for referencing in locks and interactions.',
    ),
  width: z
  ...
  data: trackDataSchema.describe('Data configuration for the track'),
  chromInfoPath: z
    .string()
    .optional()
  ...
  y: z.number().optional().describe('Vertical position coordinate in pixels.'),
  name: z.string().optional().describe('Display name for the track'),
  position: z
    .string()
    .optional()
    .describe(
      "Position of the track (e.g., 'top', 'bottom', 'left', 'right', 'center')",
    ),
  maxWidth: z.number().optional().describe('Maximum width for the track'),
  maxZoom: z.number().optional().describe('Maximum zoom level for the track'),
  binsPerDimension: z
    .number()
    .optional()
 ...
 */
export const trackSchema: z.ZodType<Track> = z.lazy(() =>
  z.union([
    baseTrackSchema,
...
export const tracksContainerSchema = z
  .object({
    left: z
      ...
      .optional()
      .describe(
        'Array of 1D vertical tracks positioned on the left side of the view.',
      ),
    top: z
      .array(trackSchema)
      .optional()
      .describe(
        'Array of 1D horizontal tracks positioned at the top of the view.',
      ),
    center: z
      .array(trackSchema)
      .optional()
    ...
    bottom: z
      ...
      .optional()
      .describe(
        'Array of 1D horizontal tracks positioned at the bottom of the view.',
      ),
    right: z
      .array(trackSchema)
      .optional()
      .describe(
        'Array of 1D vertical tracks positioned on the right side of the view.',
      ),
  })
  .describe(
    'Container for organizing tracks by their position within the view.',
  );
...
export const viewSchema = z
  .object({
    layout: viewLayoutSchema.describe(
      'Layout configuration defining the size and position of the view',
    ),
    tracks: tracksContainerSchema.describe(
      'Container for organizing tracks by their position within the view',
    ),
    uid: z
      .string()
    ...
    initialXDomain: z
      ...
      .optional()
      .describe(
        'Horizontal genomic view coordinates as [start, end]. Only necessary if the view contains top, bottom, or center tracks.',
      ),
    initialYDomain: z
      .array(z.number())
      .optional()
      .describe(
        'Vertical genomic view coordinates as [start, end]. Only necessary if the view contains left, right, or center tracks.',
      ),
    autocompleteSource: z
      .string()
      .optional()
    ...
    chromInfoServer: z
      ...
      .optional()
      .describe(
        'Server URL providing chromosome information for the genome assembly.',
      ),
    chromInfoId: z
      .string()
      .optional()
      .describe(
        'Genome assembly identifier (e.g., "hg19", "hg38", "mm10") for chromosome info.',
      ),
    chromInfoPath: z
      .string()
      .optional()
    ...
    genomePositionSearchBox: z
      .object({
        autocompleteServer: z
          ...
          .optional()
          .describe(
            'Server URL for genomic location autocomplete functionality.',
          ),
        autocompleteId: z
          .string()
          .optional()
          .describe(
            'Identifier for genomic location autocomplete functionality.',
          ),
        chromInfoServer: z
          .string()
          .optional()
      ...
      .optional()
      .describe('Configuration for the genomic position search interface.'),
    genomePositionSearchBoxVisible: z
      .boolean()
      .optional()
      .describe('Controls visibility of the genomic position search box.'),
    selectionView: z
  ...
  .describe(
    'Individual view object defining the genomic visualization layout and tracks',
  );
...
export const lockSchema = z
  ...
  .describe(
    'Synchronization configuration for coordinated interactions between views',
  );

/**
 * Zoom locks schema
 */
export const zoomLocksSchema = lockSchema.describe(
  'Zoom synchronization configuration between views.',
);

/**
 * Location locks schema
 */
export const locationLocksSchema = lockSchema.describe(
  'Location synchronization configuration between views.',
);

/**
 * Value scale locks schema
 */
export const valueScaleLocksSchema = lockSchema.describe(
  'Value scale synchronization configuration between views.',
);
...
export const higlassViewConfigSchema = z
  .object({
    editable: z
      .boolean()
      .describe(
        'Controls whether the view configuration can be edited by users. Always true in practice.',
      )
      .default(true),
    viewEditable: z
      .boolean()
    ...
    trackSourceServers: z
      .array(z.string())
      .optional()
      .describe(
        "Property 'trackSourceServers' found in HiGlass configurations.",
      ),
    views: z
      .array(viewSchema)
      .describe(
        'Array of view objects that define the genomic visualization layouts and tracks.',
      ),
