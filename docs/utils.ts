    ...
    const switchSrc = () => {
      ...
      for (const video of videos) {
        ...
        video
          ...
          ?.setAttribute('src', `https://storage.googleapis.com/mcp-web/${name}-${suffix}.mp4`);

        if (video.getAttribute('poster')) {
          video.setAttribute('poster', `background-image: url(https://storage.googleapis.com/mcp-web/${name}-${suffix}.jpg`);
        }
