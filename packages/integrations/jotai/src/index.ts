...
/**
 * React hook wrapper around addAtomTool for component-scoped tool registration.
 * Automatically handles cleanup when the component unmounts.
 */
export function useAtomTool<T>(config: AtomToolConfig<T>): void;
export function useAtomTool<T extends object>(config: ObjectAtomToolConfig<T>): void;
export function useAtomTool(config: AtomToolConfig<unknown> | ObjectAtomToolConfig<Record<string, unknown>>): void {
  useEffect(() => {
    const cleanup = addAtomTool(config);
    return cleanup;
  }, [config]);
}
